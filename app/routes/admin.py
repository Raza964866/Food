...
from .. import db
from ..models import Admin, User, MenuItem, Order, OrderItem
...
UPLOAD_FOLDER = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'app', 'static', 'images')
...
def login():
    if request.method == 'POST':
        ...
        # Check if admin exists in User table with admin role
        admin = User.query.filter_by(email=email, role='admin').first()
        
        if admin and check_password_hash(admin.password, password):
            # Use session-based authentication for admin
